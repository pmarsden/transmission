...
TorrentRendererHelper.getProgressInfo = function (controller, t) {
	...
	if (t.needsMetaData()) {
		...
		pct = Math.round(t.getRecheckProgress() * 100);
	} else if (!t.isDone()) {
	...
	if (t.isChecking()) {
		extra = 'checking';
	} else if (s === Torrent._StatusStopped) {
		extra = 'paused';
...
TorrentRendererHelper.formatETA = function (t) {
	var apct = 0;
	var tor_eta = '';
	var raw_eta = t.getETA();
	var have = t.getHave() || 0;

	var available = have;

	var size = t.getSizeWhenDone();
	var peers = t.getPeersConnected();
	var canget = t.getDesiredAvailable() || 0; 

	if (size > 0) {
		available = have + canget; 
		apct = Transmission.fmt.percentString(available/size*100.0) + "%";
	}
	...
	if (peers == 0) {
		...
		tor_eta = "N/A, Only " + apct + " Available";
	} else if (raw_eta < 0) {
		tor_eta = "Unknown";
	} else if (raw_eta < 5) {
		tor_eta = "Now";
	} else if (raw_eta > 2420000) {
		tor_eta = "Over 4 Weeks";
	} else {
		tor_eta = Transmission.fmt.timeInterval(raw_eta);
	}
...
TorrentRendererCompact.prototype = {
	...
	getPeerDetails: function (t) {
		...
		if (t.isDownloading()) {
			...
			return [s, ', ', Transmission.fmt.percentString(100.0 * t.getPercentDone()), '%'].join('');
		};
		if (t.isSeeding()) {
			var s = '';
			s +=  [ 'Ratio: ',
					 Transmission.fmt.ratioString(t.getUploadRatio()) ].join('');
			if (have_up)
				s += [ ', ',
		...
		if (t.isStopped()) {
			return ['Ratio: ', Transmission.fmt.ratioString(t.getUploadRatio()), ', ', 'Stopped' ].join('');
		};
		if (t.isChecking()) {
			return ['Verify: ', Transmission.fmt.percentString(100.0 * t.getRecheckProgress()), '%'].join('');
		};
		return t.getStateString();
	},
	...
	render: function (controller, t, root) {
		...
		$(e).toggleClass('queued', t.isQueued());
		$(e).toggleClass('paused', t.isStopped());
		$(e).toggleClass('verify', t.isChecking());
		$(e).toggleClass('download', t.isDownloading());
		$(e).toggleClass('waiting', t.isWaitingForCheck());
		$(e).toggleClass('upload', (t.getUploadSpeed() > 0));

		var eta = t.getName();
		var have_dn = t.getDownloadSpeed() > 0;

		if (have_dn) {
			  eta = eta + '  (' + TorrentRendererHelper.formatETA(t) + ')';
		};

		if (!isMobileDevice) {
			setTextContent(e, eta);
		}
		else {
